    ...
    async def async_step_init(
        ...
        if self._subentry_type == "conversation":
            ...
            matching_entities = [
                ...
                if "last_recognized_face" in entry.entity_id
            ]
            ##### Added by Home Mind #####
            step_schema.update(
                {
                    ##### Added by Home Mind #####
                    vol.Required(CONF_ASSIST_SATELLITE_ENTITY): EntitySelector(
                        EntitySelectorConfig(domain="assist_satellite")
                    ),
                    vol.Required(CONF_LAST_RECOGNIZED_FACE_ENTITY): EntitySelector(
                        EntitySelectorConfig(include_entities=matching_entities)
                    ),
                    # vol.Required(
                    #     CONF_LAST_RECOGNIZED_FACE_ENTITY,
                    #     description={
        ...
        if user_input is not None:
            ...
            user_input[CONF_LAST_RECOGNIZED_FACE_ENTITY] = user_input[
                CONF_LAST_RECOGNIZED_FACE_ENTITY
            ].strip()
            if len(user_input[CONF_LAST_RECOGNIZED_FACE_ENTITY]) > 0:
                entity = self.hass.states.get(
                    user_input[CONF_LAST_RECOGNIZED_FACE_ENTITY]
                )
                if entity is None:
                    # CONF_LAST_RECOGNIZED_FACE_ENTITY is not a valid state
                    errors[CONF_LAST_RECOGNIZED_FACE_ENTITY] = (
                        "Last recognized face entity not valid"
                    )

            user_input[CONF_ASSIST_SATELLITE_ENTITY] = user_input[
                CONF_ASSIST_SATELLITE_ENTITY
            ].strip()
            if len(user_input[CONF_ASSIST_SATELLITE_ENTITY]) > 0:
                entity = self.hass.states.get(user_input[CONF_ASSIST_SATELLITE_ENTITY])
                if entity is None:
                    # CONF_ASSIST_SATELLITE_ENTITY is not a valid entity
                    errors[CONF_ASSIST_SATELLITE_ENTITY] = (
                        "Assist satellite entity not valid"
                    )
